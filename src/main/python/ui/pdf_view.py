...
from PyQt6.QtCore import Qt, pyqtSignal, pyqtSlot, QRectF, QPointF, QSizeF, QTimer
...
class PDFGraphicsView(QWidget):
    ...
    def keyPressEvent(self, event):
        ...
        if event.key() == Qt.Key.Key_N:
            ...
            event.accept()
            return
        elif event.key() == Qt.Key.Key_Down or event.key() == Qt.Key.Key_Right or event.key() == Qt.Key.Key_Space:
            if self.current_page < self.total_pages - 1:
                self.pageChangeRequest.emit(self.current_page + 1)
                event.accept()
                return
